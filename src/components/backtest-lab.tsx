...
import { useBacktestResults } from "@/lib/use-decision-runs";
...
export function BacktestLab() {
  ...
  const [strategy, setStrategy] = useState<BacktestStrategy>({
    ...preset,
    relatedLogicChainId: searchParams.get("logic") ?? preset.relatedLogicChainId,
  });
  const [results, setResults] = useBacktestResults();
  ...
  async function runBacktest() {
    ...
    try {
      ...
      const result = response.ok
        ? ((await response.json()) as BacktestResult)
        : buildMockBacktestResult(strategy);
  ...
  return (
    <div className="space-y-8">
      <section className="grid gap-5 xl:grid-cols-[0.72fr_1.28fr]">
        ...
        />
        <ResultSummary result={selectedResult} strategy={strategy} />
      </section>
          ...
          {results.map((result) => (
            ...
            >
              <div className="flex items-center justify-between gap-3">
                <span className="text-sm font-semibold">{strategyName(result.strategyId)}</span>
    ...
    <Card className="h-fit">
      ...
      <CardContent className="space-y-4 pt-5">
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          ...
          <FormInput label="Logic Chain" value={strategy.relatedLogicChainId} onChange={(value) => setStrategy((current) => ({ ...current, relatedLogicChainId: value }))} />
        </div>
...
function ResultSummary({ result, strategy }: { result: BacktestResult; strategy: BacktestStrategy }) {
  ...
  return (
    <Card className="overflow-hidden border-primary/20">
      <CardHeader className="border-b bg-primary text-primary-foreground">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            ...
            <CardTitle className="mt-2 text-2xl text-primary-foreground">{strategyName(result.strategyId)}</CardTitle>
          </div>
      ...
      <CardContent className="space-y-5 pt-5">
        ...
        <div className="grid gap-4 border-y py-5 sm:grid-cols-3">
        ...
        <div className="flex flex-wrap gap-2">
          <Button asChild variant="outline">
            <Link href={`/committee?topic=${encodeURIComponent(strategy.name)}&signal=${strategy.relatedLogicChainId}`}>
              Committee review <ArrowRight className="size-4" />
            </Link>
          </Button>
        </div>
      </CardContent>
...
function strategyName(id: string) {
  return mockStrategies.find((strategy) => strategy.id === id)?.name ?? "Custom strategy run";
}
